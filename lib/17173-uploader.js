...
doUpload = function (uctx) {
	async.waterfall([
		...
		function (callback) {
			request.get(UPLOAD_PAGE_URL, function (error, res, body) {
				if (error) {
					...
					decoded.match(/name=\"videoUploadUrl\"\s+value=\"(http:\/\/[\w\W]+?)\"/);
			...
			function (res, body) {
				...
				if (res.statusCode === 200) {
					...
					if (result.success === 1) {
						...
						callback(null, result.data["v_id"], JSON.stringify(result.data));
					} else {
		...
		function (videoId, flashResult, callback) {
			...
			postForm(POST_VIDEO_INFO_URL,
				...
				"&intro=" + uctx.intro +
				"&v_id=" + videoId +
				"&videoUploadUrl=" + uctx.uploadUrl +
				"&big_class=1&sub_class=10278&user_ip=127.0.0.1&user_port=&needOtherVideo=0" +
				"&game-category=SD敢达OL&tags=SD敢达&is_copy=1&is_open=1&passwd=" +
