...
var logger = require("./logger.js")
	...
	/* constants*/
	, VIDEO_LIST_URL = "http://tvpot.daum.net/search/json/SearchResultJSON.do?service=clip&q=SD%EA%B1%B4%EB%8B%B4+%EB%84%A5%EC%8A%A4%ED%8A%B8&dateterm=day&size=30"
	, MOVIE_LOCATION = "http://videofarm.daum.net/controller/api/open/v1_2/MovieLocation.apixml?vid=%s&profile=HIGH&play_loc=tvpot"
	, QUICK_VIEW = "http://tvpot.daum.net/search/QuickView.do?clipid=%s"
	, VIDEO_PROVIDER = "daum"
	;
	...
	_.each(clipIds, function (clipId, i) {
		...
		waterfallFuncs.push(
			function (callbackInner) {
				logger.info("Getting video uploader. ID=" + friendlyId);

				request.get(util.format(QUICK_VIEW, friendlyId), function (error, response, body) {
					if (!error && response.statusCode === 200) {
						body.match(/<span class="etc".+?>(.+?)<\/span>/);
						var authorName = RegExp.$1,
							taskPath = context.getTaskPath(self.dataPath, self.dateString, friendlyId, VIDEO_PROVIDER),
							ctx;

						ctx = context.loadContext(taskPath);
						ctx.additionalTitleInfo = authorName;
